package com.blibli.oss.backend.newrelic.newrelic.aspect;
...
import static com.blibli.oss.backend.newrelic.newrelic.injector.NewRelicTokenInjectorFilter.TOKEN_CONTEXT_KEY;
import static com.blibli.oss.backend.newrelic.newrelic.injector.NewRelicTokenInjectorFilter.TRANSACTION_CONTEXT_KEY;
...
public class ReactiveMongoRepositoryAspectTest {
  ...
  public void before() {
    ...
    mongoProperties.setUri("mongodb://blibli.com:27017/admin");
    mongoProperties.setDatabase("testdb");
    AspectModifyService aspectModifyService = new AspectModifyServiceImpl(
        Optional.ofNullable(mongoProperties),
        Optional.ofNullable(reactiveMongoTemplate)
    );
  ...
  @Test
  public void errorOnUnsatisfiedDependencyMongoProperties() {
    AspectModifyService aspectModifyService = new AspectModifyServiceImpl(
    ...
    StepVerifier.create(source)
        .verifyErrorMatches(err ->
            err.getClass() == NoSuchBeanDefinitionException.class &&
            Objects.equals(err.getMessage(),"No bean named 'mongoProperties' available: "
                + "com.blibli.oss.backend.newrelic.newrelic-reactor plugin need MongoProperties bean to "
                + "instrument Reactive Mongo call"));

    verify(transaction).startSegment(SEGMENT_NAME_FIND_ID);
  }

  @Test
  public void errorOnUnsatisfiedDependencyMongoTemplate() {
    MongoProperties mongoProperties = new MongoProperties();
    AspectModifyService aspectModifyService = new AspectModifyServiceImpl(
        Optional.ofNullable(mongoProperties),
        Optional.ofNullable(null)
    ...
    StepVerifier.create(source)
        .verifyErrorMatches(err ->
            err.getClass() == NoSuchBeanDefinitionException.class &&
                Objects.equals(err.getMessage(),"No bean named 'mongoTemplate' available: "
                    + "com.blibli.oss.backend.newrelic.newrelic-reactor plugin need MongoTemplate bean to "
                    + "instrument Reactive Mongo call"));
